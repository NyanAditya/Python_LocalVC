...
def roomRent():
    ...
    if myConnection:
        cursor = myConnection.cursor()
        createTable = """CREATE TABLE IF NOT EXISTS ROOM_RENT(CID VARCHAR(20),ROOM_CHOICE INT,NO_OF_DAYS INT,
        ROOMNO INT ,ROOM RENT INT)"""
        cursor.execute(createTable)
...
def Restaurent():
    ...
    if myConnection:
        cursor = myConnection.cursor()
        createTable="""CREATE TABLE IF NOT EXISTS RESTAURANT(CID VARCHAR(20),CUISINE VARCHAR(30),
...
def ttlamount():
    ...
    if myConnection:
        cursor = myConnection.cursor()
        createTable = """CREATE TABLE IF NOT EXISTS TOTAL(CID VARCHAR(20),C_NAME VARCHAR(30),ROOM RENT INT ,
        RESTAURENTBILL INT ,GAMING BILL INT,FASHIONABLE INT,TOTAL AMOUNT INT)"""
        cursor.execute(createTable)
        ...
        cursor.execute("COMMIT")
        cursor.close()
        print("\n **** HOTEL ATLANTIS **** CUSTOMER BILLING ****")
        print("\n CUSTOMER NAME : ", name)
