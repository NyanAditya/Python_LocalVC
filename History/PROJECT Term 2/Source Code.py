...
mycon = sqlator.connect(host='localhost', user='root', password='Hello@Sq1')
...
# creating database
cursor.execute("create database if not exists `ddclib`")
cursor.execute("use `ddclib`")

# creating the tables we need

cursor.execute(
    "create table if not exists books(SN int(5) primary key,Book_Name varchar(100), Quantity_Available int(10),"
    "Price_Per_Day int(10))")
cursor.execute(
    "create table if not exists BORROWER(SN int(5),borrowers_name varchar(40),"
    "book_lent varchar(100),contact_no bigint(10))")
